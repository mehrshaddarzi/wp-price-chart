...
class Utility {
	...
	public static function send_mail( $to, $subject, $content ) {
		...
		//Email Template
		$email_template = wp_normalize_path( \WP_PRICE_CHART::$plugin_path . '/templates/email.php' );
		if ( trim( \WP_PRICE_CHART::$Template_Engine ) != "" ) {
			$template = wp_normalize_path( path_join( get_template_directory(), '/wp-price-chart/email.php' ) );
		...
		$opt = get_option( 'WP_PRICE_CHART_email_opt' );
