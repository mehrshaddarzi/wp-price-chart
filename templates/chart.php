<script>
	...
	$label = array();
	$data = array();
	$query = $wpdb->get_results( "SELECT `LASTSALEPRICE`, `LASTSALETIME` FROM `{$wpdb->prefix}prices` WHERE `LASTSALETIME` BETWEEN {$current_time_x_ago}000 AND {$current_time}000 ORDER BY `LASTSALETIME` ASC", ARRAY_A );
